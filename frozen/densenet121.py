...
- [Download pretrained densenet-121 model]
...
class DenseNet121:
    ...
    def __init__(self, data_format = 'channels_first', densenet_path = 'densenet121_weights_tf_dim_ordering_tf_kernels_notop.h5'):
    ...
    def _conv_layer(self, input_layer, name, strides = [1, 1, 1, 1]):
        ...
        if len(weights_key) == 4:
            weights_hierarchy = self._weights_h5[weights_key[0]][weights_key[1]][weights_key[2]][weights_key[3]][weights_key[0]][weights_key[1]][weights_key[2]][weights_key[3]] 
        else:
            weights_hierarchy = self._weights_h5[weights_key[0]][weights_key[1]][weights_key[0]][weights_key[1]]

        W = tf.constant(weights_hierarchy['kernel:0'])
    ...
    def _batchnorm_layer(self, input_layer, name):
        ...
        mean = tf.constant(weights_hierarchy['moving_mean:0'])
        std = tf.constant(weights_hierarchy['moving_variance:0'])
        beta = tf.constant(weights_hierarchy['beta:0'])
        gamma = tf.constant(weights_hierarchy['gamma:0'])
