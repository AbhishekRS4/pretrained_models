...
class ResNet50:
    ...
    def resnet50_encoder(self, features):
        ...
        # Stage 0
        self.stage0 = self._res_conv_layer(features, layer_name = 'conv1', strides = self._pool_strides)
        self.stage0 = self._get_batch_norm_layer(self.stage0, name = 'bn_conv1')
    ...
    def _get_batch_norm_layer(self, input_layer, name = 'bn'):
        return tf.layers.batch_normalization(input_layer, axis = self._bn_axis, training = self._training, name = name)
    ...
    def _res_conv_block(self, input_layer, stage, strides):
        ...
        x = self._get_batch_norm_layer(x, name = 'bn' + stage + '_branch2a')
        x = tf.nn.relu(x)

        x = self._res_conv_layer(x, layer_name = 'res' + stage + '_branch2b')
        x = self._get_batch_norm_layer(x, name = 'bn' + stage + '_branch2b')
        x = tf.nn.relu(x)

        x = self._res_conv_layer(x, layer_name = 'res' + stage + '_branch2c')
        x = self._get_batch_norm_layer(x, name = 'bn' + stage + '_branch2c')

        shortcut = self._res_conv_layer(input_layer, layer_name = 'res' + stage + '_branch1', strides = strides)
        x = self._get_batch_norm_layer(shortcut, name = 'bn' + stage + '_branch1')
