...
class InceptionV4:
    ...
    def __init__(self, data_format = 'channels_first', training = True, inception_path = 'inception_resnet_v2_weights_tf_dim_ordering_tf_kernels_notop.h5'):
    ...
    def _conv_layer(self, input_layer, strides, padding, use_bias, name):
        W_init_value = np.array(self._weights_h5[name][name]['kernel:0'], dtype = np.float32)
        ...
        if use_bias:
            b_init_value = np.array(self._weights_h5[name][name]['bias:0'], dtype = np.float32)
